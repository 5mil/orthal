...
impl NoteCommitmentTree {
    ...
    pub fn leaves(&self) -> &[[u8; 32]] { &self.leaves }
    pub fn append(&mut self, commitment: [u8; 32]) { self.leaves.push(commitment); }
    pub fn root(&self) -> [u8; 32] { merkle_root(&self.leaves) }
    ...
    pub fn verify_inclusion(leaf: [u8; 32], index: usize, path: &[[u8; 32]], root: [u8; 32]) -> bool {
        ...
        for sib in path {
            ...
            if i % 2 == 0 { c.extend_from_slice(&hash); c.extend_from_slice(sib); }
            else { c.extend_from_slice(sib); c.extend_from_slice(&hash); }
            hash = sha256d(&c);
            i /= 2;
        }
        hash == root

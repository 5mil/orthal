...
impl ActionBundle {
    ...
    pub fn is_emission(&self) -> bool {
        self.emission.is_some() && self.real_spends().is_empty() && !self.real_outputs().is_empty()
    }

    pub fn is_issuance(&self) -> bool {
        self.emission.is_none()
            && self.real_spends().is_empty()
            && self.real_outputs().iter().any(|o| !o.symbol.is_empty() || o.asset != [0u8; 32])
    }

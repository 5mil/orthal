...
#[derive(Clone, Debug)]
pub struct SpendKey { pub sk: Scalar }

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct SpendPk { pub bytes: [u8; 32] }

#[derive(Clone, Debug)]
pub struct ScanKey { pub sk: Scalar }

impl SpendKey {
    pub fn from_wallet_seed(seed: &[u8]) -> Self { Self { sk: hash_scalar(b"spend-sk", &[seed]) } }
    pub fn pk(&self) -> SpendPk {
        SpendPk { bytes: (self.sk * RISTRETTO_BASEPOINT_POINT).compress().to_bytes() }
    }
...
impl SpendPk {
    pub fn point(&self) -> Option<RistrettoPoint> { try_point(&self.bytes) }
}
impl ScanKey {
    pub fn from_wallet_seed(seed: &[u8]) -> Self { Self { sk: hash_scalar(b"scan-sk", &[seed]) } }

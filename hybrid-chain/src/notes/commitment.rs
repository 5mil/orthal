...
impl Default for PedersenGenerators {
    fn default() -> Self {
        Self {
            ...
            h: point_from_label(b"hybrid-chain-pedersen-H"),
            a: point_from_label(b"hybrid-chain-pedersen-A"),
        }
    }
}

impl PedersenGenerators {
    pub fn asset(&self) -> RistrettoPoint { self.a }
}
...
impl ValueCommitment {
    pub fn commit(value: u64, blinding: &Scalar, gens: &PedersenGenerators) -> Self {
        ...
        Self { commitment: point.compress().to_bytes() }
    }
    pub fn identity() -> Self {
        Self { commitment: RistrettoPoint::identity().compress().to_bytes() }
    }
    pub fn point(&self) -> RistrettoPoint {
        CompressedRistretto(self.commitment).decompress().unwrap_or_else(RistrettoPoint::identity)
    }
    pub fn add(&self, other: &Self) -> Self {
        Self { commitment: (self.point() + other.point()).compress().to_bytes() }
    }
...
pub fn verify_balance(inputs: &[ValueCommitment], outputs: &[ValueCommitment], fee: &ValueCommitment) -> bool {
    let mut left = RistrettoPoint::identity();
    for c in inputs { left += c.point(); }
    let mut right = fee.point();
    for c in outputs { right += c.point(); }
    left == right
}

...
impl Transaction {
    ...
    pub fn coinbase(block_height: u64, _reward: u64, dest: &[u8]) -> Self {
        Transaction {
            ...
            outputs: vec![TxOutput { value: 0, script_pubkey: dest.to_vec() }],
            locktime: 0,
            is_coinstake: false,
        }
    }
    ...
    pub fn coinstake(_stake_coins: u64, _reward: u64, dest: &[u8]) -> Self {
        Transaction {
            version: 1,
            inputs: vec![TxInput { prev_txid: [0u8; 32], prev_vout: 0, script_sig: vec![] }],
            outputs: vec![
                TxOutput { value: 0, script_pubkey: vec![] },
                TxOutput { value: 0, script_pubkey: dest.to_vec() },
            ],
            locktime: 0,
            is_coinstake: true,
        }

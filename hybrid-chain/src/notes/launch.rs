...
pub enum ProfileKind { Constrained, Sparse, Standard, Dense }

impl ProfileKind {
    pub fn cap(self) -> usize {
        match self { Self::Constrained => 64, Self::Sparse => 256, Self::Standard => 1024, Self::Dense => 4096 }
    }
    pub fn window_epochs(self) -> usize {
        match self { Self::Constrained => 4, Self::Sparse => 8, Self::Standard => 16, Self::Dense => 32 }
    }
...
pub struct LiveNote { pub cm: [u8; 32], pub pred: [u8; 32], pub pred_commit: [u8; 32] }

impl LiveNote {
    pub fn plain(cm: [u8; 32]) -> Self { Self { cm, pred: [0u8; 32], pred_commit: [0u8; 32] } }
}
...
impl LaunchSet {
    ...
    fn sorted(&self) -> Vec<[u8; 32]> { let mut l = self.live_leaves(); l.sort_unstable(); l }
    ...
    pub fn contains(&self, leaf: [u8; 32]) -> bool { self.live_leaves().iter().any(|l| *l == leaf) }
